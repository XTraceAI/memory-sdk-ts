...
import { APIPromise } from 'xtrace-memory-manager/core/api-promise';

import util from 'node:util';
import XtraceMemoryManager from 'xtrace-memory-manager';
import { APIUserAbortError } from 'xtrace-memory-manager';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['XTRACE_MEMORY_MANAGER_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: XtraceMemoryManager) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['XTRACE_MEMORY_MANAGER_LOG'] = 'debug';
      const client = new XtraceMemoryManager({
        logger: logger,
        apiKey: 'My API Key',
        bearerToken: 'My Bearer Token',
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['XTRACE_MEMORY_MANAGER_LOG'] = 'not a log level';
      const client = new XtraceMemoryManager({
        logger: logger,
        apiKey: 'My API Key',
        bearerToken: 'My Bearer Token',
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'XTRACE_MEMORY_MANAGER_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['XTRACE_MEMORY_MANAGER_BASE_URL'] = undefined;
    });
